...
export default function Home(){
 ...
 const [settings,setSettings]=useState<AppSetting>({id:1,registration_locked:false,picks_locked:false,title:'Lytle Lemon FIFA Project',game_one_label:'Game 1: Canada vs South Africa',updated_at:''});
 ...
 async function saveGamePick(participant:Participant, team:GameOneTeam, silent=false){
   if(settings.picks_locked && !isAdmin){ setStatus('Game 1 picks are locked by Admin.'); return; }
   ...
   if(!silent){ await notify([participant.phone],`Your Game 1 pick is saved: ${selected.flag} ${selected.team}.`); setStatus(`Pick saved: ${selected.flag} ${selected.team}`); }
 }
 async function register(){
   ...
   await saveGamePick(participant!,form.game_one_pick,true); await notify([participant!.phone],`Saved. Game 1 pick: ${flagFor(form.game_one_pick)} ${form.game_one_pick}.`); await pushLiveEvent('registration','New player joined',`${participant!.first_name} joined as ${participant!.flag} ${participant!.team}`);
 ...
 return <main className={displayMode?'wrap tv':'wrap'}>
  <section className="hero"><div><h1>{settings.title}</h1><p className="muted">Live bracket, leaderboard, SMS updates, Game 1 picks, whistle sounds, and admin score control.</p></div><div className="heroActions"><button className="ghost" onClick={ensureAudio}>{soundOn?'🔊 Sound On':'Enable Whistle Sound'}</button><button className="ghost" onClick={()=>setDisplayMode(!displayMode)}>{displayMode?'Exit TV Mode':'TV Display Mode'}</button><div className="badge">v1.3 • Built {buildDate}</div></div></section>
  ...
  <section className="card pickBoard"><h2>Game 1 Picks: 🇨🇦 Canada vs 🇿🇦 South Africa</h2><p className="muted">{settings.picks_locked?'Picks are locked.':'Picks are open.'} Realtime totals update for everyone.</p><div className="pickTotals"><div><b>🇨🇦 Canada</b><span>{pickCounts.Canada}</span></div><div><b>🇿🇦 South Africa</b><span>{pickCounts.SouthAfrica}</span></div></div></section>
  <div className="grid">
   {!displayMode&&<section className="card"><h2>Player Registration & Game 1 Pick</h2>{settings.registration_locked?<p className="locked">Registration is locked.</p>:<><div className="row"><input placeholder="First name" value={form.first_name} onChange={e=>setForm({...form,first_name:e.target.value})}/><input placeholder="Last name" value={form.last_name} onChange={e=>setForm({...form,last_name:e.target.value})}/></div><br/><div className="row"><input placeholder="U.S. mobile number" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}/><input placeholder="4-digit player code" maxLength={4} value={form.player_code} onChange={e=>setForm({...form,player_code:e.target.value.replace(/\D/g,'')})}/></div><p className="hint">Duplicate player codes are OK. Same phone + code lets a player update their own pick.</p><label className="muted">Tournament team</label><select value={form.team} onChange={e=>{const c=countries.find(x=>x[0]===e.target.value)!;setForm({...form,team:c[0],flag:c[1]})}}>{countries.map(c=><option key={c[0]} value={c[0]}>{c[1]} {c[0]}</option>)}</select><br/><br/><label className="muted">Game 1 pick</label><div className="choiceRow">{gameOneTeams.map(t=><button key={t.team} className={form.game_one_pick===t.team?'selectedChoice':'ghost'} disabled={settings.picks_locked} onClick={()=>setForm({...form,game_one_pick:t.team})}>{t.flag} {t.team}</button>)}</div><br/><button onClick={register}>Save / Update Picks</button></>}<p className="muted">{status}</p><div className="invite"><b>Join Link</b><span>{joinLink}</span></div></section>}
