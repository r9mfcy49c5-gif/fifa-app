...
export default function Home(){
 ...
 return <main className="wrap">
  ...
  <section className="card pickBoard"><h2>🔥 Live Now</h2>{live.length?live.map(m=><ScoreCard key={m.id} m={m}/>):<p className="muted">No match marked live yet. Admin can set one live below.</p>}</section>
  ...
  <div className="grid">
   ...
   <section className="card"><h2>🏆 Live Leaderboard</h2><div className="list">{board.map((p,i)=><div className="item" key={p.id}><span>#{i+1} {p.flag} {p.first_name} {p.last_name}<br/><span className="muted">{p.team} • W {p.wins||0} L {p.losses||0}</span></span><b>{p.points||0}</b></div>)}</div></section>
  </div>

  <section className="card"><h2>⚽ Make / Change Picks</h2>{!me&&<p className="locked">Save player first. Then pick every match here.</p>}<div className="bracket">{matches.map(m=>{const pick=me?picks.find(x=>x.participant_id===me.id&&x.match_id===m.id):undefined;return <div className="match" key={m.id}><div className="row"><b>{m.round}</b><span className="badge">{m.status}</span></div><p className="muted">{m.kickoff}</p><div className="team"><span>{m.flag_a} {m.team_a}</span><b>{m.score_a}</b></div><div className="team"><span>{m.flag_b} {m.team_b}</span><b>{m.score_b}</b></div>{m.winner&&<p className="winnerText">Winner: {m.winner}</p>}<div className="choiceRow"><button className={pick?.selected_team===m.team_a?'selectedChoice':'ghost'} onClick={()=>savePick(m,m.team_a)}>Pick {m.flag_a} {m.team_a}</button><button className={pick?.selected_team===m.team_b?'selectedChoice':'ghost'} onClick={()=>savePick(m,m.team_b)}>Pick {m.flag_b} {m.team_b}</button></div></div>})}</div></section>

  <section className="card bracketCard"><h2>📺 Match Board</h2><h3>Live</h3>{live.map(m=><ScoreCard key={`l-${m.id}`} m={m}/>)}<h3>Upcoming</h3>{upcoming.map(m=><ScoreCard key={`u-${m.id}`} m={m}/>)}<h3>Finals</h3>{finals.map(m=><ScoreCard key={`f-${m.id}`} m={m}/>)}</section>
...
function ScoreCard({m}:{m:Match}){return <div className="match"><div className="row"><b>{m.round}</b><span className="badge">{m.status}</span></div><p className="muted">{m.kickoff}</p><div className="team"><span>{m.flag_a} {m.team_a}</span><b>{m.score_a}</b></div><div className="team"><span>{m.flag_b} {m.team_b}</span><b>{m.score_b}</b></div>{m.winner&&<p className="winnerText">Winner: {m.winner}</p>}</div>}
