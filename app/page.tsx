...
export default function Home(){
 ...
 return <main className="wrap">
  ...
  <div className="grid">
   ...
   <section className="card"><h2>🏆 Live Leaderboard</h2><div className="list">{board.map((p,i)=><div className="item" key={p.id} onClick={()=>setSelectedPlayer(p)} style={{cursor:'pointer'}}><span>#{i+1} {p.flag} {p.first_name} {p.last_name}<br/><span className="muted">{p.team} • W {p.wins||0} L {p.losses||0} • Tap to view picks</span></span><b>{p.points||0}</b></div>)}</div></section>
  </div>
