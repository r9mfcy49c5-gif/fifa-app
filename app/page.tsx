...
function ScoreCard({m,picks=[]}:{m:Match;picks?:Pick[]}){const [a,b]=teamChance(m.team_a,m.team_b);const total=picks.filter(p=>p.match_id===m.id).length;const pa=pct(picks.filter(p=>p.match_id===m.id&&p.selected_team===m.team_a).length,total);const pb=pct(picks.filter(p=>p.match_id===m.id&&p.selected_team===m.team_b).length,total);return <div className="match"><div className="row"><b>{m.round}</b><span className="badge">{m.status}</span></div><p className="muted">{m.kickoff}</p><div className="team"><span>{m.flag_a} {m.team_a}</span><b>{m.score_a}</b></div><div className="meter"><i style={{width:`${a}%`}}></i></div><p className="muted">Win chance: {m.team_a} {a}% • Lytle Lemon picks {pa}%</p><div className="team"><span>{m.flag_b} {m.team_b}</span><b>{m.score_b}</b></div><div className="meter"><i style={{width:`${b}%`}}></i></div><p className="muted">Win chance: {m.team_b} {b}% • Lytle Lemon picks {pb}%</p>{m.status==='live'&&<p className="winnerText">⏱ {m.minute||0}' {m.extra_time? <span className="extraTime">+{m.extra_time}</span> : null}</p>}{m.highlights_url&&<p><a className="highlightLink" href={m.highlights_url} target="_blank">🎥 Watch Highlights</a></p>}{m.status==='live'&&<p className="winnerText">⏱ {m.minute||0}' {m.extra_time? <span className="extraTime">+{m.extra_time}</span> : null}</p>}{m.highlights_url&&<p><a className="highlightLink" href={m.highlights_url} target="_blank">🎥 Watch Highlights</a></p>}{m.status==='live'&&<p className="winnerText">⏱ {m.minute||0}' {m.extra_time? <span className="extraTime">+{m.extra_time}</span> : null}</p>}{m.highlights_url&&<p><a className="highlightLink" href={m.highlights_url} target="_blank">🎥 Watch Highlights</a></p>}{m.winner&&<p className="winnerText">Winner: {m.winner}</p>}</div>}
